...
def heure_aleatoire():
    ...
    return f"{heure:02}:{minute:02}"

def inspecter(salle):
    choix = input("Voulez-vous inspecter cette salle ? : ")
    
    if choix.lower() != "oui":
        return None

    heure = heure_aleatoire()

    if salle == "Votre chambre":
        print(
            f"Vous entrez lentement dans votre chambre. "
            f"L’air y est étonnamment glacé, bien plus que dans le reste de la maison. "
            f"La pièce est plongée dans une obscurité presque totale, seulement traversée "
            f"par une faible lumière provenant d’une horloge numérique figée sur {heure}. "
            f"Les meubles vous semblent familiers… mais quelque chose paraît profondément anormal."
        )

    elif salle == "Salle de bain des parents":
        print(
            f"Vous poussez la porte de la salle de bain. "
            f"Une légère odeur d’humidité flotte dans l’air et le miroir couvert de traces ternes "
            f"déforme vaguement votre reflet. Au-dessus du lavabo, une petite horloge murale arrêtée "
            f"sur {heure} émet un faible grésillement avant de retomber dans le silence."
        )

    elif salle == "Salle de bain":
        print(
            f"La salle de bain paraît étrangement propre, presque inutilisée. "
            f"Les carreaux froids réfléchissent la faible lumière de la pièce et rendent "
            f"l’atmosphère encore plus vide. Près de la baignoire, une horloge blanche semble "
            f"bloquée sur {heure}, comme si le temps lui-même avait cessé d’avancer ici."
        )
        

def generer_objet(etat):
    objets_inv = [
        {
            "nom": "lampe torche",
            "description": "Elle ne fonctionne pas... les piles semblent absentes."
        },
        {
            "nom": "photo de famille",
            "description": "Derrière la photo se trouve une date : 1921 et une signature étrange."
        },
        {
            "nom": "batterie",
            "description": "Des piles usées. Elles pourraient servir."
        },
        {
            "nom": "montre",
            "description": "Elle est arrêtée à une heure étrange."
        }
    ]

    objet = random.choice(objets_inv)

    for item in etat["inventaire"]:
    ...
    choix = input("Le ramasser ? (oui/non) : ")

    if choix.lower() == "oui":
        etat["inventaire"].append(objet)
        print("\nVous avez récupéré :", objet["nom"], "\n")
...
for objet in objets:
    if objet != "ordinateur":
        objets_valides.append(objet)

etat = {
    "cle_objet": random.choice(objets_valides),
    "porte_ouverte": False,
    "ordi": False,
    "post_affiche": False,
...
while True:
    ...
    if mode == "chambre":
        ...
        if salle != "Couloir" and salle != "Chambre des parents":
            inspecter(salle)
        
        if salle in ["Couloir", "Salle de bain", "Chambre des parents", "Votre chambre"]:
        ...
        if choix.lower() == "i":
            ...
            if choix in scenario[salle]["choix"]:
                ...
                if salle == "Couloir" and destination == "Escalier" and not etat["rdc"]:
                    ...
                    choix = input("Voulez-vous saisir un code ? (oui/non) : ")
    ...
    if mode == "brouillard":
        ...
        for i in range(15):
            ligne = ""
            longueur = random.randint(30, 70)

            for j in range(longueur):
                ligne += random.choice(chars)
